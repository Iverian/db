...
void MainWindow::on_add_order_clicked()
{
	QString selected_order
		= ui->lst_orders->model()->data(ui->lst_orders->currentIndex()).toString();
	int row = ui->lst_orders->currentIndex().row();
	QString selected_order_id
		= QString::number(ui->lst_orders->model()->index(row, 0).data().toInt());
	QString orders_str = ui->concat_orders_str->text();
	if (orders_str == "")
		ui->concat_orders_str->setText(selected_order_id + ":" + selected_order);
	else
		ui->concat_orders_str->setText(
			orders_str + "," + selected_order_id + ":" + selected_order);
	add_new_order();
}
...
void MainWindow::on_delete_client_order_clicked()
{
	...
	for (int i = 0; i < count; i++) {
		ui->lst_client_orders->model()->removeRow(
			ui->lst_client_orders->selectionModel()->selectedRows().at(i).row(), QModelIndex());
	}
...
void MainWindow::on_send_order_clicked()
{
	...
	for (int i = 0; i < num_orders.toInt(); i++) {
		...
		query.exec("INSERT INTO Orders(id_ordertype,id_client,numoperations) VALUES(" + IdOrderType
			+ "," + client_id + "," + num_operations + ")");
