...
QString get_staff_id(){
    ...
    if (file.open(QIODevice::ReadOnly)) return QString(file.readLine());
}
...
bool user_exist(){
    ...
    query.next();
    if (query.value(0).toInt() != 0) return true;else query.exec("UPDATE staff SET Status = 'logged_in' WHERE Id='"+staff_id+"'");
    return false;
}
