...
{
    OperationEdit::edit(this, db,
        ui->operNames->model()->index(ui->operNames->currentIndex().row(), 0).data().toInt());
    refreshOperView();
}

void MainWindow::on_actEditOrder_triggered()
{
    OrderEdit::edit(this, db,
...
{
    auto id
        = ui->staffView->model()->index(ui->staffView->currentIndex().row(), 0).data().toString();
