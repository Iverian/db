...
{
    return !contains(m_ordNames, text);
}

QString OrderEdit::getOperTitle(int operId)
{
    return std::find_if(m_oprNames->cont().begin(), m_oprNames->cont().end(),
        [&operId](
            const OperationListModel::dataType::value_type& val) { return val->id == operId; })
        ->get()
        ->data;
}
...
void OrderEdit::add(QWidget* parent, QSqlDatabase& db)
{
    if (db.isOpen()) {
        // db.transaction();
        auto edit = new OrderEdit(parent, db, -1);
...
void OrderEdit::edit(QWidget* parent, QSqlDatabase& db, int orderId)
{
    if (db.isOpen()) {
        ...
        if (!orderTitle.isEmpty()) {
            auto edit = make_unique<OrderEdit>(parent, db, orderId);
...
{
    auto startAlgoId = getFirstQueryVal<int>("SELECT last_value FROM algorithm_id_seq", m_db) + 1;
    ...
    for (auto i = 0; i != algoSize; ++i) {
        ...
        for (auto j = parentInd.begin(); j != parentInd.end(); ++j) {
            insertDependStr
                += "(%1,%2)%3"_q.arg(startAlgoId + *j)
                       .arg(startAlgoId + i)
                       .arg(std::next(j) == parentInd.end() && i + 1 == algoSize ? "" : ",");
        }
    }
    qDebug() << insertAlgoStr;
    qDebug() << insertDependStr;
...
void OrderEdit::initializePage(int id)
{
    ...
    case 1: {
        m_oprNames = OperationListModel::createFromDb(this, 0, m_db);
        m_algoModel = new AlgoModel(this);
        ui->operNames->setModel(m_oprNames);
...
{
    on_act_editParents_triggered();
}

void OrderEdit::on_operNames_doubleClicked(const QModelIndex& index)
{
    auto oper = m_oprNames->at(index.row());
    m_algoModel->appendRow({oper.id, oper.data, QSet<int>()});
}
...
void OrderEdit::on_act_editParents_triggered()
{
    ...
    for (auto i = 0, size = m_algoModel->rowCount(); i != size; ++i) {
        if (i == cur || m_algoModel->at(i).parentIndex.contains(cur))
            continue;
